...
- A standard 60-point Shield survives even a perfectly timed unmarked Fear Shot (30 base) or Dread Snipe (38 base). A marked Fear Shot breaks it with Good/Perfect timing; a marked Dread Snipe breaks it at normal timing. This makes marking the intended one-cast Shield-break setup rather than optional overkill.
- A standard 60-point Armor bar likewise survives a perfectly timed unmarked physical skill: Slash 32, Dash Slash 36, Grenade 38, and Whip Slash 30. Whip Slash deals its damage before applying Off-balance, so it cannot exploit its own mark. Off-balance doubles a later physical action's Armor pressure, allowing each skill to cross the breakpoint at normal timing.
...
- Buff/debuff commands use Skill V2's scaled-time Build Up phase and leave the caster rooted, attack-locked, and vulnerable while energy particles converge. Initial wind-ups are Motivate 0.7s, Inspire 1.0s, Mark Shot 0.65s, and Dispel 0.75s; edit `Wind Up Seconds` per skill in Character Kit Settings.
...
- All-Out becomes available only when every living enemy is knocked down. It applies up to `120` Health and `60` defense damage (provisional tuning), with Health damage capped at 80% of each target's max Health so one sweep cannot erase a full bar. It can still finish an already-damaged enemy. Every survivor stands up with full defenses afterward, even if the sweep breaks its remaining bar. Fresh breaks still award their normal rewards. A repeat requires knocking every survivor down again. It never consumes Armor's attack reward.
