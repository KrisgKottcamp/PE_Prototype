...
- Physical basic attacks deal **zero Armor/Shield damage**. They award AP on contact and damage Health when Armor is absent (including Shield-only enemies), or while the enemy is knocked down. An Armor-blocked basic does not consume the stored Armor reward.
...
- A standing enemy without Armor takes direct Health damage from physical attacks; without Shield it takes direct Health damage from magic. The unrelated defense remains untouched. Fear affects magical Health damage; mini-game defense multipliers do not boost bypass damage. Bypassing a defense grants no break reward. A health-only enemy can knock down when crossing the configured threshold.
...
- All-Out becomes available only when every living enemy is knocked down. It applies the current rules asset's `100` Health and `40` defense damage (provisional tuning). An immediate repeat requires a fresh break on every survivor. Otherwise they stay down, and another all-out setup requires a player attack to wake a survivor first. It never consumes Armor's attack reward.
...
- [ ] Confirm physical/magical routing damages both bars when the matching defense exists (100% matching / 35% off-type), but bypasses to Health when it is absent without touching the other bar. Check Fear on bypass damage, physical basic attacks against Shield-only targets, and no defense-to-Health overflow.
...
- [ ] Put every living enemy into knockdown, invoke All-Out, and verify a repeat requires fresh breaks on every survivor; verify a failed chain leaves survivors down and blocks another offer until a player wakes an enemy and sets it up again.
