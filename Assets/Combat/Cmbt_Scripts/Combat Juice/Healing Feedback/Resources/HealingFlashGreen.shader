Shader "Project Eri/Healing Flash Green"
{
    Properties
    {
        [PerRendererData] _MainTex ("Sprite Texture", 2D) = "white" {}
        _FlashColor ("Healing Color", Color) = (0.12,1,0.32,1)
    ...
    SubShader
    {
        ...
        half4 FlashFragment(Varyings input) : SV_Target
        {
            half alpha = SAMPLE_TEXTURE2D(
                ...
                sampler_MainTex,
                input.uv
            ).a * input.color.a * _FlashColor.a;

            clip(alpha - 0.001);

            half3 premultipliedColor =
                saturate(_FlashColor.rgb) * alpha;
        ...
        Pass
        {
            Name "HealingFlashUniversalForward"
